...
def load_shapenet_train(obj):
	...
	names = mat[0][0][0][0]
	objname = names[obj][0]
	print "Training objects:" + objname

	Train_size = num[obj,0]
	...
	# training data phase
	indexing = 0
	dirname = data_dir + '/' + objname + "/train"
	for i in xrange(1,int(num[obj,0])+1):
		for j in xrange(1,2):
			filename = dirname + '/' + objname + '_' + '%04d'%i + '_' + str(j) + '.mat'
	...
	return X_train,indexing

def load_shapenet_test(obj):
	theano.config.floatX = 'float32'
